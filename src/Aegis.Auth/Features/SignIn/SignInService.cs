...
namespace Aegis.Auth.Features.SignIn
{
    ...
    {
        Task<Result<SignInResponse>> SignInEmail(string email, string password, string? callback, bool rememberMe = false);
    }
    ...
    internal sealed class SignInService(AegisAuthOptions options, IAegisLogger logger, IAuthDbContext dbContext) : ISignInService
    {
        ...
        public async Task<Result<SignInResponse>> SignInEmail(string email, string password, string? callback, bool rememberMe = false)
        {
            ...
            if (!_options.EmailAndPassword.Enabled)
            {
                ...
                return Result<SignInResponse>.Failure(AuthErrors.System.FeatureDisabled, "Password auth is disabled.");
            }

            if (string.IsNullOrWhiteSpace(email))
            {
                _logger.Warning("SignIn attempt failed: Email is missing");
                return Result<SignInResponse>.Failure(AuthErrors.Validation.InvalidInput, "Email is required.");
            }
            ...
            if (!EmailValidator.Validate(normalizedEmail))
            {
                ...
                return Result<SignInResponse>.Failure(AuthErrors.Validation.InvalidInput, "Email not valid.");
            }
            ...
            catch (Exception ex)
            {
                ...
                return Result<SignInResponse>.Failure(AuthErrors.System.InternalError, "Database lookup failed.");
            }
            ...
            if (user is null)
            {
                ...
                return Result<SignInResponse>.Failure(AuthErrors.Identity.InvalidEmailOrPassword, "Invalid email or password.");
            }
            ...
            if (_options.EmailAndPassword.RequireEmailVerification && !user.IsEmailVerified)
            {
                ...
                if (_options.EmailVerification?.SendVerificationEmail is null)
                {
                    ...
                    return Result<SignInResponse>.Failure(AuthErrors.Identity.EmailNotVerified, "Email is not verified.");
                }
                ...
                if (sendOnSignIn == true || (sendOnSignIn is null && requireEmailVer == true))
                {
                    ...
                    return Result<SignInResponse>.Failure(AuthErrors.Identity.EmailNotVerified, "Verification email sent. Please check your inbox.");
                }
            ...
            if (string.IsNullOrWhiteSpace(session))
            {
                ...
                return Result<SignInResponse>.Failure(AuthErrors.System.FailedToCreateSession, "Failed to create session.");
            }

            _logger.Info("SignIn successful for user {UserId}", user.Id);

            return new SignInResponse() { User = user.ToDto(), Token = session, Redirect = !string.IsNullOrWhiteSpace(callback), Url = callback };    //Lilbro is logged in
        }
